...
- インターネットを経由しなくてもプライベートネットワーク経由でVPC外のAWSサービスへアクセス可能にする機能

### ゲートウェイ型

- Amazon S3とAmazon DynamoDBで利用できる
- S3はゲートウェイ型とPrivateLinkの両方で利用できる
- ルーティング例
  - 送信先：S3もしくはDymanoDBのプレフィックスID
  - ターゲット：VPCエンドポイントID

### AWS PrivateLink（インターフェイス型）

- ゲートウェイ型以外のサービスとつなげられる
- 有償
